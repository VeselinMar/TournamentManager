...
@pytest.mark.django_db
def test_recalculate_points_with_own_goal(auth_client, tournament, field, team):
    away = Team.objects.create(name="Away", tournament=tournament, tournament_points=3)
    team.tournament_points = 0
    team.save()
    ...
    # away scored via home own goal
    home_player = team.players.create(name="Home Player")
    MatchEvent.objects.create(
        match=match,
        event_type='own_goal',
        team=team,
    ...
    # now add a regular home goal to make it 1-1
    url = reverse('add-match-event', kwargs={
        'tournament_id': tournament.pk,
        'match_id': match.pk
    })
    ...
    # own_goal counts toward away score, regular goal counts toward home
    # result should be 1-1 draw
